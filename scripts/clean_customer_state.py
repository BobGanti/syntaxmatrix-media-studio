...
CUSTOMERS_FILE = ROOT / "billing" / "customers.js"
...
def _should_remove(row: Any) -> bool:
    ...
    return (
        ...
        or "stage9" in blob
        or name.startswith("stage9")
        or "mock" in customer_id
    )


def clean_customer_state(print_summary: bool = True) -> dict[str, list[dict[str, str]]]:
    if not CUSTOMERS_FILE.exists():
        CUSTOMERS_FILE.parent.mkdir(parents=True, exist_ok=True)
        CUSTOMERS_FILE.write_text("[]\n", encoding="utf-8")
        result = {"removed": [], "kept": []}
        if print_summary:
            print("customers.js did not exist; created clean empty list.")
        return result
...
if __name__ == "__main__":
    clean_customer_state(print_summary=True)
