...
SYSTEM_SERVICE = ROOT / "services" / "clone_voice_system.py"
...
required = [WORKSPACE_SERVICE, SYSTEM_SERVICE, CONTROLLER, HTML, JS, CSS]
...
for path in required:
    backup = path.with_name(path.name + f".bak.step1-stable-naming-{stamp}")
...
def safe_slug(value: str | None, fallback: str = "untitled") -> str:
    value = (value or "").strip()
    value = pathlib.Path(value).stem if value else fallback
...
def get_workspace(workspace_id: str | None = None) -> WorkspacePaths:
    ...
    paths.voice_previews_dir.mkdir(parents=True, exist_ok=True)
...
def voice_id_from_source_filename(filename: str | None) -> str:
    stem = safe_slug(filename, fallback="recorded_voice")

    for suffix in ("_parameter", "_preview"):
        if stem.endswith(suffix):
            stem = stem[: -len(suffix)]

    return stem or "recorded_voice"


def narration_filename_from_title(title: str | None) -> str:
    slug = safe_slug(title, fallback="narration")
    return f"{slug}_{timestamp()}.wav"


def stable_parameter_path(paths: WorkspacePaths, voice_id: str) -> pathlib.Path:
    safe_voice_id = safe_slug(voice_id, fallback="voice")
    return paths.voice_params_dir / f"{safe_voice_id}_parameter.txt"


def stable_preview_path(paths: WorkspacePaths, voice_id: str, source_filename: str | None) -> pathlib.Path:
    safe_voice_id = safe_slug(voice_id, fallback="voice")
    suffix = pathlib.Path(secure_filename(source_filename or "source.wav")).suffix or ".wav"
    return paths.voice_previews_dir / f"{safe_voice_id}_preview{suffix}"
...
def save_source_audio(file: FileStorage, paths: WorkspacePaths) -> pathlib.Path:
    ...
    target = paths.tmp_source_audio_dir / f"tmp_{timestamp()}_{stem}{ext}"
    file.save(target)
    return target


def save_stable_preview(
    paths: WorkspacePaths,
    source_audio_path: pathlib.Path,
    source_filename: str | None,
    voice_id: str,
    overwrite: bool = False,
) -> pathlib.Path:
    preview_path = stable_preview_path(paths, voice_id, source_filename)

    if preview_path.exists() and not overwrite:
        return preview_path

    shutil.copyfile(source_audio_path, preview_path)
    return preview_path
...
def voice_parameter_exists(paths: WorkspacePaths, voice_id: str) -> bool:
    return stable_parameter_path(paths, voice_id).exists()


def load_voice_parameter(paths: WorkspacePaths, voice_id: str) -> tuple[str, pathlib.Path]:
    param_path = stable_parameter_path(paths, voice_id)

    if not param_path.exists() or not param_path.is_file():
        raise FileNotFoundError(f"Voice parameter not found: {voice_id}")

    value = param_path.read_text(encoding="utf-8").strip()

    if not value:
        raise RuntimeError(f"Voice parameter file is empty: {param_path}")
...
def save_voice_parameter(paths: WorkspacePaths, voice_parameter: str, voice_id: str) -> tuple[str, pathlib.Path]:
    safe_voice_id = safe_slug(voice_id, fallback="voice")
    param_path = stable_parameter_path(paths, safe_voice_id)
    param_path.write_text(voice_parameter.strip(), encoding="utf-8")
    return safe_voice_id, param_path


def list_workspace_voice_parameters(paths: WorkspacePaths) -> list[dict[str, str | None]]:
    ...
    for param_path in sorted(paths.voice_params_dir.glob("*.txt"), key=lambda p: p.stat().st_mtime, reverse=True):
        ...
        voice_id = stem[:-10] if stem.endswith("_parameter") else stem

        preview_url = None
        for ext in ("wav", "mp3", "m4a", "ogg", "webm"):
            candidate = paths.voice_previews_dir / f"{voice_id}_preview.{ext}"
            if candidate.exists() and candidate.is_file():
                preview_url = f"/media/workspaces/{paths.workspace_id}/voice_previews/{candidate.name}"
                break

        voices.append({
            "voiceId": voice_id,
            "displayName": display_name_from_voice_id(voice_id),
...
def load_workspace_voice_parameter(paths: WorkspacePaths, requested_voice_id: str) -> tuple[str, pathlib.Path]:
    safe_voice_id = safe_slug(requested_voice_id, fallback="voice")
    return load_voice_parameter(paths, safe_voice_id)


def generated_audio_path(paths: WorkspacePaths, title: str | None = None) -> pathlib.Path:
    return paths.generated_audio_dir / narration_filename_from_title(title)
...
SYSTEM_SERVICE.write_text(r'''from __future__ import annotations
...
def _voice_id_from_param_path(param_path: pathlib.Path) -> str:
    stem = param_path.stem
    return stem[:-10] if stem.endswith("_parameter") else stem
...
def load_system_voice_parameter(voice_id: str) -> tuple[str, pathlib.Path]:
    safe = pathlib.Path(voice_id).name.strip()

    candidates = [
        SYSTEM_PARAMS_DIR / f"{safe}_parameter.txt",
        SYSTEM_PARAMS_DIR / f"{safe}.txt",
    ]

    param_path = next((path for path in candidates if path.exists() and path.is_file()), None)
...
from services.clone_voice_workspace import (
    ...
    load_voice_parameter,
    relative_to_root,
    save_source_audio,
    save_stable_preview,
    save_voice_parameter,
    stable_parameter_path,
    ...
    if "clone_voice_from_source" not in app.view_functions:
        ...
        def from_source():
            ...
            try:
                ...
                preview_path = save_stable_preview(
                    workspace,
                    source_path,
                    audio_file.filename,
                    voice_id,
                ...
                print("[clone_voice_controller] Stable preview path:", preview_path, flush=True)

                if voice_parameter_exists(workspace, voice_id):
                    voice_parameter, param_path = load_voice_parameter(workspace, voice_id)
                    print("[clone_voice_controller] Reusing existing stable voice parameter:", param_path, flush=True)
                else:
                    ...
                    print("[clone_voice_controller] Stable voice parameter saved:", param_path, flush=True)
...
<head>
  ...
  <link rel="stylesheet" href="/clone_voice/client.css?v=stable-naming-1">
</head>
<body>
  <main class="shell">
    <header class="hero">
      ...
      <form id="cloneVoiceForm">
        ...
        <div class="panel" id="recordPanel" hidden>
          <div class="inner-card">
            ...
            <p class="status" id="recordingStatus">Start recording, speak clearly, then stop. The recording will be sent as recorded_voice.wav.</p>
        ...
        <label>
          Narration title
          <input id="titleInput" name="title" type="text" required placeholder="Example: LinkedIn open source article">
        </label>
...
input[type="file"], input[type="text"], textarea { width: 100%; border: 1px solid #465662; border-radius: 12px; padding: 12px; background: #202b35; color: #fff; }
...
JS.write_text(r'''(() => {
  ...
  function renderSimpleVoiceList({ listEl, voices, selectedVoice, type }) {
    ...
    listEl.innerHTML = voices.map((voice, index) => {
      ...
      const previewButton = voice.previewUrl
        ? `<button class="secondary icon-button" type="button" data-${type}-play-index="${index}" title="Play preview" aria-label="Play preview">▶</button>`
        : "";
  ...
  savedVoiceList.addEventListener("click", async (event) => {
    const playButton = event.target.closest("[data-saved-play-index]");
    const useButton = event.target.closest("[data-saved-use-index]");

    if (playButton) {
      const index = Number(playButton.dataset.savedPlayIndex);
      await playPreview(savedVoices[index], index, playButton);
      return;
    }
  ...
  function stopPreview() {
    ...
    document.querySelectorAll("[data-system-play-index], [data-saved-play-index]").forEach((button) => {
      button.textContent = "▶";
  ...
  async function playPreview(voice, index, button) {
  ...
  systemVoiceList.addEventListener("click", async (event) => {
    ...
    if (playButton) {
      ...
      await playPreview(systemVoices[index], index, playButton);
      return;
    }
  ...
  async function stopRecording() {
    try {
      ...
      recordedFilename = "recorded_voice.wav";
  ...
  async function submitUploadOrRecord(title, prompt) {
    let fileOrBlob = null;
    let filename = "";
    ...
    if (sourceMode === "upload") {
      ...
      filename = recordedFilename || "recorded_voice.wav";
    }
  ...
  async function submitSaved(title, prompt) {
  ...
  async function submitSystem(title, prompt) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!title) {
      alert("Enter narration title first.");
      return;
    }
    ...
    try {
      ...
      if (sourceMode === "upload" || sourceMode === "record") {
        response = await submitUploadOrRecord(title, prompt);
      }

      if (sourceMode === "saved") {
        response = await submitSaved(title, prompt);
      }

      if (sourceMode === "system") {
        response = await submitSystem(title, prompt);
      }
  ...
  console.log("[Clone Voice] stable naming client loaded");
})();
''', encoding="utf-8")

for py_file in [WORKSPACE_SERVICE, SYSTEM_SERVICE, CONTROLLER]:
    py_compile.compile(str(py_file), doraise=True)

print()
print("STEP 1 COMPLETE: stable voice naming + title-based narration filenames.")
print()
print("New private voice naming:")
print("  source bobga.wav       -> bobga_parameter.txt + bobga_preview.wav")
print("  source bobga_2.mp3     -> bobga_2_parameter.txt + bobga_2_preview.mp3")
print("  recorded voice         -> recorded_voice_parameter.txt + recorded_voice_preview.wav")
print()
print("New narration naming:")
print("  <title>_YYYYMMDDHHMMSS.wav")
print()
print("Restart Flask:")
print("  python app.py")
print()
print("Open:")
print("  http://127.0.0.1:5055/tasks/clone-voice?stable-naming=1")
