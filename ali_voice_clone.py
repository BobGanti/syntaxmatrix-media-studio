...
if __name__ == '__main__':
    ...
    SOURCE_VOICE_FILE_NAME = "smxPP.wav"  ###xxxxxxxxxxxx######
    ...
    preview_text = read_text_from_file(preview_fname)   
    
    narration_fname = "txt_sources/thank_u.txt" ###########
    narration_text = read_text_from_file(narration_fname) 
    ...
    ext = SOURCE_VOICE_FILE_NAME.split(".")[-1]
    if MODE == preview:
        output_filename = f"{CLONED_DATA_FILE_NAME}_preview.{ext}"
        ...
        outId = datetime.now().strftime("%Y%m%d%H%M%S")
        output_filename = f"{title}_{outId}.{ext}"
